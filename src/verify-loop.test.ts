...
describe("verifyWithRetry", () => {
  ...
  test("retry prompt builder receives cumulative prior attempts on second retry", async () => {
    ...
    const promptBuilderCalls: Array<{
  ...
  test(".hone-gates.json in folder overrides opts.gates passed to gateRunner", async () => {
    ...
    const overrideGates: GateDefinition[] = [{ name: "override-test", command: "bun test", required: true }];

    await writeFile(join(folder, ".hone-gates.json"), JSON.stringify({ gates: overrideGates }));
