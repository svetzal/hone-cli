...
import { CliError, errorMessage, warn } from "./errors.ts";
...
export async function listAgents(agentsDir?: string): Promise<AgentInfo[]> {
  ...
  try {
    ...
    if (code !== "ENOENT") {
      warn(`Failed to read agents directory ${dir}: ${errorMessage(err)}`);
    }
    return [];
  }
}
...
export async function readAgentContent(name: string, agentsDir?: string): Promise<string | null> {
  ...
  try {
    ...
    warn(`Failed to read agent file ${agent.file}: ${errorMessage(err)}`);
    return null;
  }
}
